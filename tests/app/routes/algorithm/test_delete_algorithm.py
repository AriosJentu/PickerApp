...
class BaseTestDeleteAlgorithm(BaseTestSetup):
    ...
    async def _send_delete_request(self, client_async: AsyncClient, algorithm: BaseObjectData[Algorithm], headers: InputData | None = None) -> Response:
...
class TestDeleteAlgorithm(BaseTestDeleteAlgorithm):
    ...
    async def test_delete_algorithm_success_moderators(self,
        ...
        response = await self._send_delete_request(client_async, algorithm, base_user_other.headers)
        json_data = response.json()
